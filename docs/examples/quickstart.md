...
To read the contents of a specific file inside the opened archive, you can open the `SgaFile` entry and read its stream.
...
If you want to update an existing archive, open it in `SgaMode.Write` mode, modify entries, then dispose the archive to write the changes bach to the file.
