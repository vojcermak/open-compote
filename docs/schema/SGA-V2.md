...
2. Drive count - Count of drives in the archive.
...
Offsets represent the count of bytes from the start of the TOC header. (Toc starts directly after the data offset field, at position 180.) 
...
3. First folder - Inclusive index into the folder definition list to the first folder in the drive.
4. Last folder -  Exclusive index into the folder definition list to the last folder in the drive.
5. First file - Inclusive index into the file definition list to the first file in the drive.
6. Last file - Exclusive index into the file definition list to the last file in the drive.
7. Root folder - Index into the folder definition list for this drive root folder. 
...
3. Last folder - Exclusive index into the folder definition list to the last subfolder in the folder.
4. First file - Inclusive index into the file definition list to the first file in the folder.
5. Last file - Exclusive index into the file definition list to the last file in the folder.

> Folder names are stored as full paths, but file names are stored as names only. For example, a folder name could look like this `art/ebps`. Where the `ebps` is the name of the folder and this folder is located in the `art` folder. But file `image.png` in the `ebps` folder has only `image.png` name stored.
...
> The difference between the Impossible Creatures archives and DOW1 archives is the size of the storage flag: Dawn of War uses 4 bytes, whereas Impossible Creatures uses only 1 byte.
...
- 0 - Uncompress - Raw uncompressed data
- 16 - Buffer Compressed - ZLIB compressed data
- 32 - Stream Compressed - ZLIB compressed data

Both the Buffer Compressed and Stream Compressed Storage options store the data using the ZLIB compression algorithm. Probably the only difference is that the game handles the file differently.

### Name list
Name list is an Array of ASCII(UTF-8) null-terminated strings(C strings) representing the names of all files and folders in the archive.

## Data Block
The data block contains the actual content of archived files. 

### File Metadata
File metadata is a block of information about a file directly **before** the file contents of a file. It is 264 bytes long and contains the file name, last change date and file crc.
...
3. CRC - CRC-32 hash, calculated from the uncompressed file contents.

According to this [source](https://mak-relic-tool.github.io/Schema-Documentation/sga/v2/data.html#file-metadata). The file metadata are not required by the game engine. From what I found, W40k Dawn of War accepts any sgas regardless of what is in the File metadata part. I tested it with both my files generated by Open Compote, which at that point only filled the file metadata with 0, and with sga generated using the official modding tools, where the file metadata are missing entirely.
