...
> Folder names are stored as full paths, but file names are stored as names only. For example folder name could look like this `art/ebps`. Where the `ebps` is the name of the folder and this folder is located in the `art` folder. But file `image.png` in the `ebps` folder have only `image.png` name stored.
...
Both the Buffer Compressed and Stream Compressed Storage options stores the data using the same ZLIB compression algorithm. Probably only difference is that the game handles the file differently.
